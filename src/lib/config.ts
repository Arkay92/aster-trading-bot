...
const envSchema = z.object({
  ...
  EMERGENCY_FLATTEN_ON_RISK_HALT: envBoolean.optional(),
  TRADING_DAY_TIMEZONE_OFFSET_MINUTES: z.coerce.number().int().min(-720).max(840).optional(),
  REQUIRE_FRESH_PERP_SIGNALS: envBoolean.optional(),
  PERP_SIGNAL_MAX_AGE_MS: z.coerce.number().int().positive().optional(),
  ORDER_RECONCILE_INTERVAL_MS: z.coerce.number().int().positive().optional(),
  ...
  MAX_ORDER_AGE_MS: z.coerce.number().int().positive().optional(),
...
export const loadConfig = (overrides?: Partial<AppConfig>): AppConfig => {
  ...
  const risk: RiskConfig = {
    ...
    emergencyFlattenOnRiskHalt: env.EMERGENCY_FLATTEN_ON_RISK_HALT ?? true,
    tradingDayTimezoneOffsetMinutes: env.TRADING_DAY_TIMEZONE_OFFSET_MINUTES ?? 0,
    requireFreshPerpSignals: env.REQUIRE_FRESH_PERP_SIGNALS ?? true,
    perpSignalMaxAgeMs: env.PERP_SIGNAL_MAX_AGE_MS ?? 10 * 60_000,
    orderReconcileIntervalMs: env.ORDER_RECONCILE_INTERVAL_MS ?? 30_000,
    execution: {
      ...
      maxOrderAgeMs: env.MAX_ORDER_AGE_MS ?? 5 * 60_000,
    },
  };
