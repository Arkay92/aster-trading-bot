...
export const loadConfig = (overrides?: Partial<AppConfig>): AppConfig => {
  ...
  const risk: RiskConfig = {
    ...
    regimeAdxThreshold: env.REGIME_ADX_THRESHOLD ?? 25,
    requireStructureBreak: env.REQUIRE_STRUCTURE_BREAK ?? true,
    structureLookbackBars: env.STRUCTURE_LOOKBACK_BARS ?? 5,
    requireVolumeSpike: env.REQUIRE_VOLUME_SPIKE ?? false,
    volumeSpikeMultiplier: env.VOLUME_SPIKE_MULTIPLIER ?? 1.3,
    atrLength: env.ATR_LENGTH ?? 14,
    atrStopMultiplier: env.ATR_STOP_MULTIPLIER ?? 1.5,
    atrTakeProfitR: env.ATR_TAKE_PROFIT_R ?? 2,
    moveStopToBreakevenR: env.ATR_BREAKEVEN_R ?? 1,
